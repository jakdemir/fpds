...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
		...
		neighborsWithHistory(neighbors.head._1, neighbors.head._2).filter{case (block, moves) => !explored.contains(block)}
	}
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = {
			...
			if(initial.isEmpty){
				...
				val more = for {
					path <- initial
					next <- newNeighborsOnly(Stream(path), explored ++ initial.map(_._1))
				} yield next
				initial ++ from(more, explored ++ more.map(_._1) ++ initial.map(_._1))
			}
	}
